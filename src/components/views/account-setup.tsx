...
export const resetPassword = async (
  ...
  const response = await fetchBaseClient(
    `/api/auth/reset-password?token=${token}&setup=true`,
    {
      method: "POST",
      body: JSON.stringify({
...
export const verifyUserAccount = async (token: string) => {
  const response = await fetchBaseClient(`/api/auth/verify?token=${token}`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
    },
...
interface AccountSetupPageViewProps {
  token: string;
}

export function AccountSetupPageView({ token }: AccountSetupPageViewProps) {
  ...
  const {
    isFetching: isVerifyAccountFetching,
    isSuccess: isVerifyAccountSuccess,
    isError: isVerifyAccountError,
  } = useQuery({
    queryKey: ["verifyUserAccount", token],
    queryFn: () => verifyUserAccount(token),
    refetchOnWindowFocus: false,
    enabled: !!isResetPasswordSuccess,
  });
  ...
  }, [isVerifyAccountSuccess, countdown, router]);

  if (isFetching || isVerifyAccountFetching)
    return (
  ...
  if (isError || isVerifyAccountError) {
    return (
      <div className="bg-transparent mb-20 flex flex-col items-center pt-10 px-4">
