...
import UploadDevelopmentForm from "./upload-development-form";
import { Dialog, DialogContent, DialogTitle } from "@/components/ui/dialog";

export default function UploadDevelopmentModal({
  open,
  handleOpenChange,
}: {
  open: boolean;
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="[&>button]:hidden max-w-[95%] w-[552px] rounded-2xl max-h-[95%] overflow-y-auto">
        ...
        <p className="text-xs text-[#26045D]">
          If you can’t find the development in the list, you will need to
          request a new development to be created. In order to do this, please
          download the below CSV and provide the necessary information.{" "}
        </p>
        <Link
          href="https://zuxxygfnipgdaeirxagh.supabase.co/storage/v1/object/public/stairpay//Developments_Template_2025_07_10.csv"
          target="_blank"
        ...
        <UploadDevelopmentForm
          onDevelopmentAdded={() => handleOpenChange(false)}
        />
      </DialogContent>
    </Dialog>
  );
