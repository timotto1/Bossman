...
export default function UnitListingEligibilityCard() {
  ...
  useEffect(() => {
    if (!isLoading && unitListing) {
      form.reset({
        criteria: unitListing?.criteria,
        minimumDeposit: unitListing?.minimum_deposit,
        minimumShare: unitListing?.minimum_share,
      });
    }
