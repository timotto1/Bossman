...
export default function ListingAttachmentsPage() {
  const { toast } = useToast();

  const { data: developmentListing, refreshListing } = useDevelopmentListing();
  ...
  const handleRemove = async () => {
    ...
    try {
      ...
      const { error } = await supabase
        .from("development_listing_documents")
        .delete()
        .in("id", idsToDelete);
