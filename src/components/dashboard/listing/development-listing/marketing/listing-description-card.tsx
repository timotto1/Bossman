...
export default function ListingDescriptionCard() {
  ...
  const form = useForm<DescriptionFormData>({
    ...
    mode: "onChange",
    defaultValues: {
      keyInformationURL: developmentListing?.key_information_url,
      description: developmentListing?.development_description,
    },
  });
  ...
  const generateListingDescription = async () => {
    ...
    try {
      const response = await fetch(`/api/listing/generate-description`, {
        ...
        body: JSON.stringify({
          listingId: developmentListing?.id,
          type: "developments",
        }),
      });
  ...
  const handleSubmit = async (data: DescriptionFormData) => {
    ...
    try {
      await updateListing({
        ...
        development_description: data.description!,
      });
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
          ...
          <div className="flex items-center justify-end w-full gap-2">
            <Button
              disabled={isUpdating || isGeneratingDescription}
              type="submit"
