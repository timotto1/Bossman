...
export class Result<T, E extends ResultErr = Error> {
  ...
  /**
   * Replaces the contained value with the given value if the Result is an Ok, or
   * does nothing if it is an Err.
   * @param value
   * @returns Result
   */
  map<NewT>(value: (value: T) => NewT): Result<NewT, E> {
    if (this._error) {
      return this.errInto();
    }
    return Result.fromOk<NewT, E>(value(this._value));
  }
}
